...
def shift_tokens_right(input_ids: torch.Tensor, pad_token_id: int) -> torch.Tensor:
    """Shift input ids one token to the right (inline replacement for transformers utility)."""
...
def parse_arguments(parser: argparse.ArgumentParser):
    ...
    parser.add_argument('--gpus', action='store_true', default=torch.cuda.is_available())
...
def train(config: Config, train_dataloader: DataLoader, num_epochs: int, early_stop: int, 
    ...
    if config.fp16:
        scaler = torch.amp.GradScaler('cuda', enabled=bool(config.fp16))
    ...
    modeldict = torch.load('model.pth', weights_only=True)
...
def test(config: Config, valid_dataloader: DataLoader, model: nn.Module, dev: torch.device,
    ...
    with torch.no_grad(), torch.amp.autocast('cuda', enabled=False):
        for index, batch in tqdm(enumerate(valid_dataloader), desc="--validation", total=len(valid_dataloader)):
            ...
            generated_ids = _model.generate(batch.input_ids.to(dev), batch.flag.to(dev))
            '''
    ...
    for pred, gold in zip(predictions_text, targets_text):
        rouge_score += _rouge_scorer.score(gold, pred)['rougeL'].fmeasure
...
def main():
    ...
    if opt.mode == "train":
        ...
        device_ids = list(range(torch.cuda.device_count()))
        if conf.gpus and torch.cuda.is_available() and len(device_ids) > 0:
            model = nn.DataParallel(model, device_ids=device_ids, output_device=0)
        ...
        if conf.gpus and torch.cuda.is_available() and len(device_ids) > 0:
            model = nn.DataParallel(model, device_ids=device_ids, output_device=0)
        modeldict = torch.load(f'model.pth', weights_only=True)
        model.load_state_dict(modeldict)
