...
# ---------------------------------------------------------------------------
# State classes  (previously from fastNLP.modules.decoder.seq2seq_state)
# ---------------------------------------------------------------------------

class State:
    """Base state object carrying encoder output and decode progress."""

    def __init__(self, encoder_output=None, encoder_mask=None):
    ...
    def reorder_state(self, indices):
        if self.encoder_output is not None:
            self.encoder_output = self.encoder_output.index_select(0, indices)
        if self.encoder_mask is not None:
            self.encoder_mask = self.encoder_mask.index_select(0, indices)


class LSTMState(State):
    """State object for LSTMSeq2SeqDecoder."""

    def __init__(self, encoder_output, encoder_mask, hidden, cell):
        super().__init__(encoder_output, encoder_mask)
        self.hidden = hidden
        self.cell = cell
        self.input_feed = encoder_output.new_zeros(
            encoder_output.size(0), encoder_output.size(-1)
        )

    def reorder_state(self, indices):
        super().reorder_state(indices)
        self.hidden = self.hidden.index_select(1, indices)
        self.cell = self.cell.index_select(1, indices)
        self.input_feed = self.input_feed.index_select(0, indices)


class TransformerState(State):
    """State object for TransformerSeq2SeqDecoder (inference key/value caching)."""
    ...
    def reorder_state(self, indices):
        super().reorder_state(indices)
        for i in range(len(self.decoder_prev_key)):
            if self.decoder_prev_key[i] is not None:
                self.decoder_prev_key[i] = self.decoder_prev_key[i].index_select(0, indices)
                self.decoder_prev_value[i] = self.decoder_prev_value[i].index_select(0, indices)
            if self.encoder_key[i] is not None:
                self.encoder_key[i] = self.encoder_key[i].index_select(0, indices)
                self.encoder_value[i] = self.encoder_value[i].index_select(0, indices)


# ---------------------------------------------------------------------------
# Attention modules  (previously from fastNLP.modules.attention)
# ---------------------------------------------------------------------------

class MultiHeadAttention(nn.Module):
    """Standard multi-head attention compatible with TransformerState kv-caching."""
    ...
    def forward(self, query, key, value, key_mask=None, attn_mask=None, state=None):
        ...
        if isinstance(state, TransformerState):
            if qkv_same:
                # Self-attention: generation always feeds the full sequence each step,
                # so we do NOT accumulate prev_k (that would give k_len > q_len and
                # break the causal mask). Always recompute KV from the current tokens.
                pass
            else:
        ...
        if isinstance(state, TransformerState):
            if not qkv_same:
                # Cache encoder (cross-attention) KV — encoder output is constant.
                state.encoder_key[self.layer_idx] = k
                state.encoder_value[self.layer_idx] = v

        batch_size, q_len, _ = query.size()
        k_len = k.size(1)
        q = q.reshape(batch_size, q_len, self.n_head, self.head_dim).transpose(1, 2)
        k = k.reshape(batch_size, k_len, self.n_head, self.head_dim).transpose(1, 2)
        v = v.reshape(batch_size, k_len, self.n_head, self.head_dim).transpose(1, 2)

        attn_weights = torch.matmul(q, k.transpose(-2, -1))  # b, n, q, k
        ...
        output = torch.matmul(attn_weights, v).transpose(1, 2).reshape(batch_size, q_len, self.d_model)
        output = self.out_proj(output)
        return output, attn_weights


class AttentionLayer(nn.Module):
    """Additive attention for LSTMSeq2SeqDecoder (not used by Bert2tf, kept for completeness)."""

    def __init__(self, query_size, key_size, value_size):
        super().__init__()
        self.key_proj = nn.Linear(key_size, query_size, bias=False)
        self.out_proj = nn.Linear(query_size + key_size, value_size)

    def forward(self, query, key, key_mask=None):
        # query: (batch, query_size)  key: (batch, src_len, key_size)
        scores = torch.bmm(key, self.key_proj(query).unsqueeze(-1)).squeeze(-1)  # batch, src_len
        if key_mask is not None:
            scores = scores.masked_fill(~key_mask.bool(), -float('inf'))
        attn = F.softmax(scores, dim=-1)
        context = torch.bmm(attn.unsqueeze(1), key).squeeze(1)  # batch, key_size
        combined = torch.cat([query, context], dim=-1)
        return self.out_proj(combined), attn


# ---------------------------------------------------------------------------
# Embedding helpers  (previously from fastNLP.embeddings)
# ---------------------------------------------------------------------------

class StaticEmbedding(nn.Module):
    """Stub — not used in the Bert2tf execution path."""
    pass


def get_embeddings(init_embed):
    """Return an nn.Embedding from either an nn.Module or a (vocab_size, embed_dim) tuple."""
    if isinstance(init_embed, nn.Module):
        return init_embed
    elif isinstance(init_embed, tuple):
        num_embeddings, embedding_dim = init_embed
        return nn.Embedding(num_embeddings, embedding_dim)
    raise TypeError(f"Unsupported type for init_embed: {type(init_embed)}")
...
class FLAGMultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, flag, key_mask=None, attn_mask=None, state=None):
        ...
        # During inference the flag has only 1 row (is_training=False);
        # pad with zeros to match current query length.
        if flag.size(1) < q_len:
            ...
            flag = flag[:, :q_len, :]
        rel_k = self.k_flag(flag.long()) #batch x q_len x k_len x dim
        #print(rel_k.size())
        ...
        rel_k = torch.matmul(rel_k.permute(1,0,2,3,4), q.transpose(2,3)) # kbqnn
        rel_k = torch.mean(rel_k, dim=-1) #kbqn
        rel_k = rel_k.permute(1,2,0,3)
        score = torch.einsum('bqnh,bknh->bqkn', q, k)  # bs,q_len,k_len,n_head
        #flag_score = torch.einsum('bqnh,bknh->bqkn', q, rel_k)  # bs,q_len,k_len,n_head
